-- Open the comprobante to all staff, not just the technical admin.
--
-- The document layout is settled and in real use, so the temporary
-- "tech-admin only while it's being refined" gate on issue_comprobante is
-- dropped: any authenticated staff member can now issue/send a comprobante
-- (the owner and her helpers all work orders from the panel). Everything else
-- about the function is unchanged — still SECURITY DEFINER, still idempotent,
-- still one live numbered document per order, still granted only to the
-- `authenticated` role (anon cannot call it).
